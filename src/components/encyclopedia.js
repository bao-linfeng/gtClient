...
class Encyclopedia extends Component {
    ...
    componentDidMount(){
        ...
        let uri=self.props.planlist.api+('news/list?keyword='+encodeURI(keyword)+'&type='+encodeURI('百科')+'&sort='+encodeURI(sortType)+'&page=1&limit=10');
    ...
    handleGetDataForPage(e,type){
        ...
        let uri=self.props.planlist.api+('news/list?keyword='+encodeURI(keyword)+'&type='+encodeURI('百科')+'&sort='+encodeURI(sortType)+'&page='+pager+'&limit=10');
        let getConsultation=function(response){
            console.log(response);
    ...
    render() {
        ...
        return (
            <div className="expertConsultation" onLoad={(e)=>{store.dispatch(setPathname('/encyclopedia'))}}>
                <Header />
                <div className="expertConsultation_search" style={{backgroundImage:'url(./images/ex_other_bg.jpg)'}}>
                                    ...
                                    <div key={key} className={(key == tabIndex ? 'show' : '')+' expertConsultation_list'}>
                                        {
                                            ...
                                            expertConsultationList.map((item2,key2)=>{
                                                return (
                                                    <Link to={'/encyclopediaDetail:'+item2._id} key={key2} className='expertConsultation_list_a'>
                                                        <h3>{item2.title}</h3>
