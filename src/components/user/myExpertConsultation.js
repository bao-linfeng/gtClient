...
class MyExpertConsultation extends Component {
    constructor(props) {
        super(props);
        this.state = {
            userId:'',
    ...
    componentWillMount(){
        ...
        let url=this.props.planlist.api+'myExpertConsultation/list?userId='+userId+'&isAnswer='+isAnswer+'&page=1&limit='+limit;
    ...
    handleGetDataForPage(e,type){
        ...
        let url=this.props.planlist.api+'myExpertConsultation/list?userId='+userId+'&isAnswer='+isAnswer+'&page='+pager+'&limit='+limit;
    ...
    handleDelete(e,id){
        ...
        e.preventDefault();
        let url=this.props.planlist.api+'myExpertConsultation/delete';
    ...
    render() {
        ...
        return (
            <div className="userBox" onLoad={(e)=>{store.dispatch(setPathname('/myExpertConsultation'))}}>
                <Header />
                <div className='w_1170 userCenter'>
                    ...
                    <div className='userInfo_content' style={{paddingBottom:'66px',position:'relative'}}>
                        <div className='user_title'>
                            <span>我的咨询</span>
                            <Link to='/addConsultation' className='updateGenBtn'>我要咨询</Link>
                        </div>
                        <div className='userInfo_form' style={{paddingTop:'20px'}}>
                        {
                            ...
                            genealogyList.map((item,key)=>{
                                return (
                                    <Link to={'/expertConsultationDetail:'+item._id} key={key} className='my_expertConsultation_list'>
                                        <h3>{item.title}</h3>
                                        <Tag color={switchType(item.type)}>{item.type}</Tag>
...
export default connect(mapStateToProps)(MyExpertConsultation);